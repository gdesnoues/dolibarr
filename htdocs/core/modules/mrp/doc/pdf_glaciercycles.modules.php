...
/**
 *	\file       htdocs/core/modules/mrp/doc/pdf_glaciercyles.modules.php
 *	\ingroup    mrp
 *	\brief      File of class to generate MO document from GlacierCycles model
 */
 ...
 */
class pdf_glaciercycles extends ModelePDFMo
{
	/**
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('desc')) {
						$pdf->startTransaction(); //description
						$des = '<b>' . $prod -> label . '</b><br>' .$prod -> description;
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell(100, 3, $object->LibStatut($object->status), '', 'R');
        ...
        if (!empty($object->date_end_planned)) {
            $posy += 5;
            $pdf->SetFont('', '', $default_font_size);
            $pdf->SetXY($posx, $posy);
            $pdf->SetTextColor(0, 0, 60);
            $pdf->MultiCell($w, 3, "Date d'échéance : ".dol_print_date($object->date_end_planned, 'day', false, $outputlangs, true), '', 'R');
        }
        ...
        $pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("Client"), 0, $ltrdirection);
        $pdf->Rect($posx, $posy, $widthrecbox, $frameheight);
        ...
        $pdf->SetFont('', '', $default_font_size - 2);
        $pdf->SetXY($posx + 2, $posy - 5);
        $pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("Product"), 0, $ltrdirection);
        $pdf->Rect($posx, $posy, $widthrecbox, $frameheight);
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 3, $prodToMake->ref, 0, $ltrdirection);
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 3, $prodToMake->label, 0, $ltrdirection);
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->writeHTMLCell(0, 0, $posx, $posy, $prodToMake->description, 0, 1, 0, true, 'L');
		$posy = $pdf->GetY();
